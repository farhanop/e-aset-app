...
export function MasterItemForm({ initialData, onSave, onCancel }: FormProps) {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({ kode_item: '', nama_item: '', id_kategori: '', metode_pelacakan: '', umur_ekonomis: '' });
    }
    setErrors({});
  }, [initialData]);
  ...
  const validateForm = () => {
    ...
    } else if (formData.kode_item.length > 50) {
      newErrors.kode_item = "Kode item maksimal 50 karakter";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const dataToSubmit = {
        ...
        umur_ekonomis: formData.umur_ekonomis ? Number(formData.umur_ekonomis) : undefined,
      };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        <label htmlFor="id_kategori" className={`block text-sm font-medium ${
      ...
      <div>
        <label htmlFor="metode_pelacakan" className={`block text-sm font-medium ${
          theme === "dark" ? "text-gray-300" : "text-gray-700"
        }`}>
      ...
      <div>
        <label htmlFor="kode_item" className={`block text-sm font-medium ${
          theme === "dark" ? "text-gray-300" : "text-gray-700"
        }`}>
      ...
      <div>
        <label htmlFor="nama_item" className={`block text-sm font-medium ${
          theme === "dark" ? "text-gray-300" : "text-gray-700"
        }`}>
      ...
      <div>
        <label htmlFor="umur_ekonomis" className={`block text-sm font-medium ${
          theme === "dark" ? "text-gray-300" : "text-gray-700"
        }`}>
