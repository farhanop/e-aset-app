...
interface AssetDetail {
  ...
  file_dokumen?: string | null; // Tambahkan field untuk dokumen
  item?: any;
  lokasi?: any;
  unitKerja?: any;
  group?: any;
...
function AssetMainInfo({
  ...
  return (
    <div
      className={`rounded-lg shadow-md p-6 ${getThemeClass(
        theme,
        "bg-gray-800",
        "bg-white"
      )}`}
    >
      <h2
        className={`text-xl font-semibold mb-4 ${getThemeClass(
          theme,
          "text-white",
          "text-gray-800"
        )}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p className="text-base">{asset.item?.nama_item || "N/A"}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-base">{asset.merk || "-"}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-base">{asset.tipe_model || "-"}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-base">{formatDate(asset.tgl_perolehan)}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-base">{asset.sumber_dana || "-"}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-base">{asset.nomor_urut}</p>
        </div>
      </div>
      ...
      <div className="mt-4">
        ...
        <p className="text-base">{asset.spesifikasi || "-"}</p>
      </div>
    </div>
  );
}
...
function AssetStatus({ asset, theme }: { asset: AssetDetail; theme: string }) {
  const statusColor = useMemo(
    () => getStatusColor(asset.status_aset, theme),
    [asset.status_aset, theme]
  );
    ...
    >
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <p className="text-base">{asset.kondisi_terakhir || "-"}</p>
        </div>
      </div>
    </div>
  );
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <p className="text-base">{asset.lokasi?.nama_ruangan || "N/A"}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-base">{asset.lokasi?.kode_ruangan || "N/A"}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-base">{asset.lokasi?.lantai || "-"}</p>
        </div>
    ...
    >
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-base">{asset.unitKerja?.nama_unit || "N/A"}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-base">{asset.unitKerja?.kode_unit || "N/A"}</p>
        </div>
...
// Komponen untuk informasi group aset
    ...
    >
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-base">{asset.group.nama_group || "N/A"}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-base">{asset.group.kode_group || "N/A"}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-base">{asset.group.deskripsi || "-"}</p>
        </div>
      </div>
    </div>
  );
...
// Komponen untuk dokumen dan media terkait
function AssetDocuments({
  ...
  onViewDocument,
  previewImage,
  onViewPdf, // Tambahkan properti untuk melihat PDF
  pdfLoading, // Tambahkan properti untuk status loading PDF
  pdfError, // Tambahkan properti untuk error PDF
}: {
  asset: AssetDetail;
  theme: string;
  onPrintQR: () => void;
  ...
  onViewPdf: (e: React.MouseEvent) => void; // Tambahkan properti
  pdfLoading: boolean; // Tambahkan properti
  pdfError: string | null; // Tambahkan properti
}) {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div>
          <p
            className={`text-sm font-medium mb-2 ${getThemeClass(
              theme,
              "text-gray-400",
              "text-gray-500"
            )}`}
          ...
          {asset.file_qrcode ? (
            <div className="flex flex-col items-start">
              <QRCodeDisplay asset={assetData} size={120} className="mb-3" />
              <button
                onClick={onPrintQR}
                className={`px-4 py-2 rounded-md flex items-center ${getThemeClass(
                  theme,
        ...
        <div>
          ...
          {asset.file_dokumen ? (
            <>
              <div className="mb-3 flex items-center p-2 rounded border border-gray-300 dark:border-gray-600">
                <svg
                ...
                <div className="text-sm">
                  <p className="font-medium truncate max-w-xs">
                    {asset.file_dokumen.split("/").pop() || "Dokumen.pdf"}
                  </p>
...
export function AssetDetailPage() {
  ...
  const handlePrintQR = useCallback(async () => {
    ...
    try {
      ...
      if (!printWindow) {
        setPrintError(
          "Browser memblokir jendela popup. Silakan izinkan popup untuk fitur ini."
        );
        return;
      }
      ...
      const assetForPrint = {
        ...
        tipe_model: asset.tipe_model || "",
        lokasi: asset.lokasi
          ? { nama_ruangan: asset.lokasi.nama_ruangan }
          : undefined,
  ...
  const handleViewDocument = useCallback(
    async (e: React.MouseEvent) => {
      ...
      try {
        // Gunakan API instance yang sudah terkonfigurasi dengan auth
        const response = await api.get(asset.foto_barang, {
          responseType: "blob", // Penting untuk mendapatkan data sebagai blob
        });
        ...
        let errorMessage = "Gagal mengakses foto. Silakan coba lagi nanti.";

        if (err.response) {
          if (err.response.status === 401) {
            errorMessage = "Sesi Anda telah berakhir. Silakan login kembali.";
          } else if (err.response.status === 404) {
            errorMessage = "Foto tidak ditemukan di server.";
          } else if (err.response.status === 403) {
            errorMessage = "Anda tidak memiliki izin untuk mengakses foto ini.";
          }
        } else if (err.message) {
  ...
  // Handler untuk melihat PDF - Tambahkan fungsi ini
  const handleViewPdf = useCallback(
    async (e: React.MouseEvent) => {
      e.preventDefault();
      if (!asset?.file_dokumen) return;
      ...
      try {
        // Gunakan API instance yang sudah terkonfigurasi dengan auth
        const response = await api.get(asset.file_dokumen, {
          responseType: "blob", // Penting untuk mendapatkan data sebagai blob
        });
        ...
        // Tampilkan pesan error yang lebih spesifik
        let errorMessage = "Gagal mengakses dokumen. Silakan coba lagi nanti.";

        if (err.response) {
          if (err.response.status === 401) {
            errorMessage = "Sesi Anda telah berakhir. Silakan login kembali.";
          } else if (err.response.status === 404) {
            errorMessage = "Dokumen tidak ditemukan di server.";
          } else if (err.response.status === 403) {
            errorMessage =
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      if (err.response?.status === 400) {
        errorMessage =
          err.response?.data?.message || "Tidak dapat menghapus aset ini.";
      } else if (err.response?.status === 401) {
        errorMessage = "Sesi Anda telah berakhir. Silakan login kembali.";
      } else if (err.response?.status === 404) {
  ...
  if (loading) {
    return (
      <div
        className={`p-6 rounded-lg shadow-md ${getThemeClass(
          theme,
          "bg-gray-800",
          "bg-white"
        )}`}
  ...
  if (error) {
    return (
      ...
      >
        <div
          className={`p-4 mb-4 rounded-md ${getThemeClass(
            theme,
            "bg-red-900/20",
            "bg-red-50"
          )}`}
        ...
        <div className="flex space-x-3">
          <button
            ...
            className={`px-4 py-2 rounded-md ${getThemeClass(
              theme,
  ...
  if (!asset) {
    return (
      ...
      >
        ...
        <Link
          to="/assets"
          className={`mt-4 inline-block px-4 py-2 rounded-md ${getThemeClass(
            theme,
            "bg-blue-600 hover:bg-blue-700",
            "bg-blue-500 hover:bg-blue-600"
          )} text-white`}
  ...
  return (
    <div>
      ...
      {printError && (
        <div
          className={`mb-6 p-4 rounded-md ${getThemeClass(
            theme,
            "bg-yellow-900/20",
            "bg-yellow-50"
          )}`}
        >
          <div className="flex">
            <svg
              className={`w-5 h-5 mr-2 mt-0.5 ${getThemeClass(
                theme,
                "text-yellow-400",
                "text-yellow-500"
              )}`}
      ...
      <div
        className={`grid grid-cols-1 lg:grid-cols-3 gap-6 ${getThemeClass(
          theme,
          "text-gray-300",
          "text-gray-700"
        )}`}
      >
        ...
        <div className="lg:col-span-3">
          <AssetDocuments
            ...
            onViewPdf={handleViewPdf} // Tambahkan ini
            pdfLoading={pdfLoading} // Tambahkan ini
            pdfError={pdfError} // Tambahkan ini
          />
        </div>
      </div>
      ...
      <div className="mt-6">
        <Link
          to="/assets"
          className={`px-4 py-2 rounded-md inline-flex items-center ${getThemeClass(
            theme,
            "bg-gray-700 hover:bg-gray-600",
            "bg-gray-200 hover:bg-gray-300"
          )}`}
      ...
      {showDeleteModal && (
        ...
        >
          <div className="p-4">
            <p
              className={`mb-6 ${getThemeClass(
                theme,
                "text-gray-300",
                "text-gray-700"
              )}`}
            >
              Apakah Anda yakin ingin menghapus aset ini? Tindakan ini tidak
              dapat dibatalkan.
            </p>
